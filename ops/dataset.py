...
class TSNDataSet(data.Dataset):
    def __init__(self, root_path, list_file,
                 ...
                 remove_missing=False, dense_sample=False, twice_sample=False, window=None):
    ...
    def _sample_indices(self, record):
        ...
        if self.dense_sample:  # i3d dense sample
            ...
            average_duration = (self.window) // self.num_segments
            if record.num_frames <= self.window:
                offsets = np.zeros(self.num_segments,)
                for index in range(self.num_segments):
                    if (index + 1) * record.num_frames / self.window > 1:
                        offsets[index] = 0
                    else:
                        offsets[index] = index * average_duration
                return offsets + 1
            else:
                randStardPoint = randint(record.num_frames - self.window, size=1)            
                offsets = np.array(range(0, self.window, average_duration)) \
                    + randStardPoint
                return offsets + 1
        else:  # normal sample
    ...
    def get(self, record, indices):
        ...
        return process_data, self.gen_label(record.label)
    
    def gen_label(self, recordLabel):
        digitString = str(recordLabel)
        labelVector = np.zeros(4)
        
        for index in range(len(digitString)):
            labelVector[int(digitString[index])] = 1
